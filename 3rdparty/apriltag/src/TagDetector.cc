...
std::vector<TagDetection> TagDetector::extractTags(const cv::Mat &image) {
    // convert to internal apriltag image (todo: slow, change internally to OpenCV)
    int width = image.cols;
    int height = image.rows;
    apriltag::FloatImage fimOrig(width, height);
    int i = 0;
...
#pragma omp parallel for
...
#if 0
  {
    ...
    // cout << "apriltag: edges=" << nEdges << " clusters=" << clusters.size() << " segments=" << segments.size()
